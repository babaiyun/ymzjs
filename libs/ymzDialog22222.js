...
(function(global){
    ...
    function ensureStyle(){
        ...
        style.textContent =
            ...
            '.ymz-dialog-x{border:0;background:none;padding:0 2px;font-size:22px;line-height:1;cursor:pointer;color:#777}' +
    ...
    function getCancelValue(option){
        return option.cancelValue == null ? null : option.cancelValue;
    }
    ...
    function focusDefault(item){
        ...
        if(
            doc.activeElement &&
            item.box.contains(doc.activeElement)
        ){
            return;
        }
    ...
    function closeItem(item, value){
        ...
        if(typeof item.option.onClose === 'function'){
            try{
                item.option.onClose(value);
            }catch(e){
                reportError(e);
            }
        }
    ...
    function close(value){
        ...
        return closeItem(current, value);
    }
    ...
    function open(option){
        ...
        if(current){
            closeItem(current, getCancelValue(current.option));
        }
        ...
        var id ='ymz_dialog_' + (++seq),
            ...
            buttons =Array.isArray(option.buttons) ? option.buttons : [],
            resolveFunc,
            promise,
            item,
            i;
        ...
        title.textContent =option.title == null ? '提示' : String(option.title);

        box.setAttribute('aria-labelledby', title.id);

        x.type ='button';
        x.className ='ymz-dialog-x';
        ...
        for(i=0; i<buttons.length; i++){
            if(!buttons[i] || typeof buttons[i] !== 'object'){
                throw new TypeError('dialog button must be an object');
            }
            ...
            (function(button){
                ...
                btn.textContent =button.text == null
                    ? '确定'
                    : String(button.text);
                ...
                btn.onclick =function(){
                    ...
                    if(typeof button.onClick === 'function'){
                        try{
                            result =button.onClick(box, button.value);
                        }catch(e){
                            reportError(e);
                            return;
                        }
        ...
        x.onclick =function(){
            closeItem(item, getCancelValue(option));
        };
        ...
        if(option.closeOnEsc !== false){
            ...
            doc.addEventListener('keydown', item.keyHandler, true);
        }
    ...
    function promptDialog(message, option){
        ...
        input.value =option.value == null ? '' : String(option.value);
        ...
        controller =open({
            title:option.title || '请输入',
            contentNode:wrap,
            width:option.width,
            cancelValue:null,
            closeOnEsc:option.closeOnEsc,
            closeOnMask:option.closeOnMask,
    ...
    global.ymzDialog ={
        ...
        current:function(){
            return current ? current.box : null;
        }
    };
