...
(function(global){
     ...
     * 若运行环境只实现其中一个，统一退化到 setTimeout，避免句柄无法正确取消。
     */
    ...
    function extend(target){
        ...
        for(var i=1,len=arguments.length; i<len; i++){
            ...
            for(var name in source){
                if(name !== '__proto__' && hasOwn(source, name)){
    ...
    function each(value, callback){
        ...
        if(isArrayLike(value)){
            ...
            for(name in value){
                if(
                    hasOwn(value, name) &&
                    callback.call(value[name], value[name], name, value) === false
                ){
                    break;
                }
            }
    ...
    /**
     * 浅复制常用基础数据类型。
     * Array / PlainObject 为浅复制；Date / RegExp 创建独立实例；其他保持原值。
     */
    function clone(value){
        if(isArray(value)) return value.slice();

        if(isPlainObject(value)){
            var result =objectGetPrototypeOf &&
    ...
    function queryElem(elem, root){
        ...
        var queryRoot =getRoot(root);

        return queryRoot && typeof queryRoot.querySelector === 'function'
            ? queryRoot.querySelector(elem)
            : null;
    }
    ...
    function queryAll(selector, root){
        ...
        return queryRoot && typeof queryRoot.querySelectorAll === 'function'
            ? toArray(queryRoot.querySelectorAll(selector))
            : [];
    }
    ...
    function dataset(elem, name, defVal){
        var defaultValue =defVal === undefined ? null : defVal;
        ...
        if(typeof elem.getAttribute === 'function'){
            value =elem.getAttribute(dataAttrName(name));
    ...
    function datasets(elem, names){
        ...
        // 单个字段名按一个字段处理，避免字符串被当成 ArrayLike 逐字符遍历。
        if(typeof names === 'string') names =[names];
        if(!isArrayLike(names)) return result;

        for(var i=0,len=names.length; i<len; i++){
            result[names[i]] =dataset(elem, names[i]);
        }
    ...
    function matches(elem, selector){
        ...
        var fn =elem.matches || elem.msMatchesSelector || elem.webkitMatchesSelector;
        return !!(fn && fn.call(elem, selector));
    }

    function closest(elem, selector){
        if(!elem) return null;
        if(elem.nodeType !== 1) elem =elem.parentElement;
        if(!elem) return null;
    ...
    function eventClosest(event, selector, root){
        ...
        return typeof root.contains === 'function' && root.contains(elem)
            ? elem
            : null;
    }
    ...
    function normalizeWait(wait){
        wait =Number(wait);
        return isFinite(wait) && wait > 0 ? wait : 0;
    }
    ...
    function uniqueId(prefix){
        uid++;
        return (prefix == null ? 'ymz_' : String(prefix)) + uid;
    }
    ...
    /**
     * 注入一次性样式。
     * 同 ID STYLE 已存在时复用；若 ID 被其他元素占用则返回 null，避免重复 ID。
     */
    function injectStyle(id, css){
        ...
        if(elem){
            if(String(elem.tagName || '').toLowerCase() === 'style') return elem;
        ...
        if(configData.styleNonce){
            elem.setAttribute('nonce', configData.styleNonce);
        }

        elem.textContent =String(css);
        (doc.head || doc.documentElement).appendChild(elem);
        return elem;
    }
    ...
    /**
     * 尽量取得 JSON.stringify 当前 holder/key 对应的原始属性值。
     * accessor 不主动再次读取，避免 safe 编码额外触发一次 getter。
     */
    function getJsonRawValue(holder, key, fallback){
        if(!objectGetOwnPropertyDescriptor || holder == null) return fallback;

        try{
            var desc =objectGetOwnPropertyDescriptor(holder, key);
            if(desc && hasOwn(desc, 'value')) return desc.value;
        }catch(e){}
     ...
     * 本函数不会为了识别原始值而额外执行一次 getter。
     */
    function jsonEncodeSafe(value){
        var stack =[];

        return JSON.stringify(value, function(key, val){
            var raw =getJsonRawValue(this, key, val),
                rawType =typeof raw;

            if(rawType === 'undefined') return null;
            if(rawType === 'function') return '[Function]';
            if(rawType === 'bigint') return raw.toString();
            if(rawType === 'symbol') return String(raw);
